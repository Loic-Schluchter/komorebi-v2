  ...
  return (
    <section>
      <div className="py-4 flex justify-between items-center text-[0.8rem] uppercase tracking-wider">
        ...
        <button className="font-serif italic flex items-center gap-2 text-lg" aria-label="See all" onClick={() => (router.push("/cities"))}>
      ...
      <ul className="flex gap-4 overflow-x-auto snap-x snap-mandatory -mx-6 px-6 pb-8 noScrollbar" ref={carouselRef}>
        {cities.slice(0, maxItems).map((city, index) => (
          ...
          >
            <Image src={city.image} alt={city.name} fill  className="object-cover" priority quality={50} />
            ...
            <Button ariaLabel="next" onClick={()=> handleClick(city.name)}>
              <MoveRight size={20} />
            </Button>
          </li>
        ))}
